...
keeps eager/SDPA QSA as the correctness default, adds an opt-in Triton QSA
indexer, and makes the unsupported Ulysses path fail explicitly. MTP is
intentionally outside the training model and is filtered by
...
config.add_post_import_block(
    ...
    # keeps eager/SDPA QSA by default while expert, loss, GDN, and QSA paths
    # can opt into VeOmni kernels.
...
# 1. Keep the exact eager reference as the default path.
# 2. Let the opt-in Triton backend split packed varlen at cu_seq_lens_q.
# ================================================================
@config.override_method(
...
# 1. Thread packed boundaries into the QSA indexer.
# ================================================================
@config.override_method(
