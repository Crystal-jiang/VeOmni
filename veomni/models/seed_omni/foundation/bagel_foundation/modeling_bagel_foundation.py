...
from .taylorseer import cal_type, derivative_approximation, taylor_cache_init, taylor_formula
...
class PackedAttention(Qwen2Attention):
    def __init__(self, config, layer_idx: Optional[int] = None):
        ...
        # transformers v5 Qwen2Attention no longer stores these on `self`; the packed
        # forward paths below rely on them, so set them explicitly from the config.
...
class PackedAttentionMoT(Qwen2Attention):
    def __init__(self, config: BagelFoundationConfig, layer_idx: Optional[int] = None):
        super().__init__(config, layer_idx)
        self.hidden_size = config.hidden_size
        self.num_heads = config.num_attention_heads
        self.num_key_value_heads = config.num_key_value_heads
...
class BagelFoundationModel(BaseFoundationModelMixin, Qwen2ForCausalLM):
    config_class = BagelFoundationConfig

    def __init__(self, config: BagelFoundationConfig, **kwargs):
        super().__init__(config, **kwargs)

    def forward(self, *args, **kwargs):
        # BagelFoundationModel is the packed "language model" of the Bagel composition.
        # Orchestration (building the packed sequence, scattering ViT/VAE embeddings, and
        # computing the CE/MSE losses) lives in SeedOmniModel for the bagel path; here we
        # simply dispatch to the packed Qwen2ForCausalLM forward (train vs inference).
        return Qwen2ForCausalLM.forward(self, *args, **kwargs)
